 ...
 * @return int
 */
Phidget.prototype.forceBetween = function(value, min, max) {
...
PhidgetInterfaceKit.prototype.setOutput = function(index, value) {
  ...
  value = (value === true);
  index = self.forceBetween(index, 0, 7);
...
PhidgetLED.prototype.setBrightness = function(index, value) {
  ...
  index = [].concat(index);
  value = self.forceBetween(value, 0, 100);

  for (var i = 0; i < index.length; i++) {
    var pos = self.forceBetween(index[i], 0, 63);
