...
phidgets.prototype.setOutput = function(output, value){
  var self = this;
  ...
  if(value === 1 || value === 0){
    var msg = 'set /PCK/PhidgetInterfaceKit/' + self.data.id + '/Output/' + output + '="' + value + '"' + this.options.delimiter;
  ...
  while((index = self.socketDataString.indexOf('\n')) > -1){
    ...
    if(words[0] === "report" && words[4] === 'key'){
      ...
      if(type == "Input"){
        ...
        self.emit("sensor", number, value);
      } else if(type == "Output"){
        self.data.outputs[number] = value;
        self.emit("output", number, value);
      } 

      if (self.id == null){
        self.id = parseInt(kitId);
      }
    }else if(words[0] === '994'){
