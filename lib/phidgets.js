...
Phidget.prototype.open = function(options) {
  ...
  self._client = net.createConnection(self.port, self.host, function() {
    ...
    // only when the first occurrence of the following line is received: "report 200-that's
    // all for now". If that line is not received within a certain delay, we must issue a
    // timeout.
