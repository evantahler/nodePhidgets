 ...
 * @todo create getter/setter for interReportPeriod. it cannot be set after the connection
 *
 ...
 *
 * @param chunk {String} A UTF-8 encoded string received from the Phidget WebService
 * @private
 */
...
Phidget.prototype._parseReport = function(line, self) {
  ...
  // Grab the serial as soon as we see it. This is also our first chance to initialize the
  // board with whatever desired settings
  if (
    line.indexOf('report 200-lid0 is pending, key /PSK') > -1 &&
    !self.serial &&
    parseInt(parts[3]) > 0
  ) {
    self.serial = parseInt(parts[3]);

    // We use this as an anchor point to give specific boards the chance to set initial
    // board state.
...
/**
 * Returns a /PCK string built from the specified parameters.
 *
 ...
 * @method _parsePhidgetSpecificReport
 * @param line {String} The raw line received from the webservice.
 * @param parts {Array} The /PCK data split on the "/" character.
 * @private
 */
PhidgetInterfaceKit.prototype._parsePhidgetSpecificReport = function (line, parts) {
  ...
  } else if (parts[4] && parts[4].indexOf('Ratiometric') === 0) {

    self._ratiometric = parts[4].split(" ")[3].replace('"', "");
