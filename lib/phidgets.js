...
Phidget.prototype._parseReport = function(line, self) {
  ...
  if (type === 'Input' || type === 'Output' || type === 'Sensor') {
    ...
    if (type === "Input"){
      ...
      self.data.inputs[index] = (value === 1);
  ...
  } else if (type && type === 'Status latest value "Detached" (removing)') {
...
/**
 * Set the specified output to open (true) or closed (false).
 *
 * @method setOutput
 * @param {int} index The output number to set
 * @param {int} value The value (true/false) you wish to set the output to
 * @returns {Phidget} Returns the Phidget to allow method chaining
 */
PhidgetInterfaceKit.prototype.setOutput = function(output, value) {
...
PhidgetLED.prototype.setBrightness = function(index, value) {
  ...
  self._sendKey(self._makeIndexedKey('Brightness', index, 63), value, true);
  self.data.outputs[index] = value;
