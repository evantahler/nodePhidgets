 ...
 * @todo Understand what lid0 and random id do. I do not see them used anywhere ?!
...
Phidget.prototype.open = function(options) {
  ...
  self._client = net.createConnection(self.port, self.host, function() {
    ...
    var message  = "set /PCK/Client/0.0.0.0/" + Math.floor(Math.random() * 99999) + "/";
    message += self.type;
    if (self.label) {
...
Phidget.prototype._handleConnectionEnd = function() {
  ...
  if(self.reopen === true && self._reopenCount < self._reopenMaxCount) {
    ...
    // Instead of trying to reconnect right away, we give a little for the network or
    // device to come back
    setTimeout(
      function() {
        /**
 ...
 * @param persistent {Boolean=false} Whether the value should persist or whether its for
 *        the session only.
 * @private
 */
...
var PhidgetInterfaceKit = function() {
   ...
   * are integers between 0 and 1000. To retrieve a value simply use the sensor number.
   * For example: `PhidgetInterfaceKit.sensors[5]`.
   *
   * @property sensors
   * @type {Object}
   */
  ...
  /**
   * [read-only] An object containing the current raw and unadjusted value for all board
   * sensors. The values are integers between 0 and 4096. To retrieve a value simply use
   * the sensor number. For example: `PhidgetInterfaceKit.sensorsRaw[5]`.
   *
   * @property sensorsRaw
   * @type {Object}
   */
  ...
  /**
   * [read-only] An object containing the current update intervals (in milliseconds) for
   * all sensors. To retrieve a value simply use the sensor number. For example:
   * `PhidgetInterfaceKit.sensorsUpdateIntervals[5]`. You cannot change the update
   * interval by changing values in this array. To change a sensor's update interval, use
   * `PhidgetInterfaceKit.setUpdateInterval()`.
   *
   * @property sensorsUpdateIntervals
   * @type {Object}
   */
  this.sensorsUpdateIntervals = {};

  /**
   * [read-only] An object containing the current sensitivity threshold for all sensors.
   * The threshold is measured in `sensorvalue` (0-1000). It is the smallest change that
   * will trigger an update notification from the sensor. Sensitivity threshold and update
   * intervals are mutually exclusive. If you set the sensitivity of a sensor, the update
   * interval will be ignored and vice versa.
   *
   * To retrieve a value simply use the sensor number. For example:
   * `PhidgetInterfaceKit.sensorsSensitivities[5]`. You cannot change the sensitivity by
   * changing values in this array. To change a sensor's sensitivity, use
   * `PhidgetInterfaceKit.setSensitivity()`.
   *
   * @property sensorsSensitivities
   * @type {Object}
   */
  this.sensorsSensitivities = {};
  ...
  /**
   * [read-only] The number of outputs supported on this device.
   *
   * @property numberOfOutputs
   * @type {int}
   */
  this.numberOfOutputs = undefined;

  /**
   * [read-only] The number of analog sensor inputs supported by this device.
   *
   * @property numberOfSensors
   * @type {int}
   */
  this.numberOfSensors = undefined;

  /**
   * [read-only] The number of digital inputs supported by this device.
   *
   * @property numberOfInputs
   * @type {int}
   */
  this.numberOfInputs = undefined;
 ...
 * can only be called once the board is 'opened'.
 *
 * @method setUpdateInterval
 * @param index {int|Array} The sensor's number (or an array of sensor numbers)
 * @param [value] {int} The number of milliseconds you wish to set the interval to.
 * @returns {PhidgetInterfaceKit} Returns the PhidgetInterfaceKit to allow method
 *          chaining.
 */
PhidgetInterfaceKit.prototype.setUpdateInterval = function(index, value) {
  ...
  for (var i = 0; i < index.length; i++) {
    var pos = parseInt(index[i]);
    self.sensorsUpdateIntervals[pos] = value;
 ...
 * set the sensitivity.
 *
 * Note that this function can only be called once the board is 'opened'.
 *
 * @method setSensitivity
 * @param index {int|Array} The sensor's number (or an array of sensor numbers)
 * @param [value] {int} The number sensitivity threshold to assign (0-1000)
 * @returns {PhidgetInterfaceKit} Returns the PhidgetInterfaceKit to allow method
 *          chaining.
 */
PhidgetInterfaceKit.prototype.setSensitivity = function(index, value) {
  ...
  for (var i = 0; i < index.length; i++) {
    var pos = parseInt(index[i]);
    self.sensorsSensitivities[pos] = value;
...
PhidgetInterfaceKit.prototype._parsePhidgetSpecificData = function (data) {
  ...
  if (data.keyword === "Input") {
     ...
     */
    self.sensorsRaw[data.index] = parseInt(data.value);
    self.emit(
      "sensor",
      self,
    ...
    self.numberOfInputs = parseInt(data.value);
  } else if (data.keyword === 'NumberOfOutputs') {
    self.numberOfOutputs = parseInt(data.value);
  } else if (data.keyword === 'NumberOfSensors') {
    self.numberOfSensors = parseInt(data.value);
  } else if (data.keyword === 'DataRate') {
    self.sensorsUpdateIntervals[data.index] = parseInt(data.value);
    ...
    self.sensorsSensitivities[data.index] = parseInt(data.value);
